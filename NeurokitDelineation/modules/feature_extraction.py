...
class ECGFeatureExtractor:
    ...
    def extract_features(self):
        dropped_idxs = [] # keep a list of indices that throw an exception
        features_list = []
        channel_1_idx = self.channel_seq.index(self.channel)

        for idx, ecg_sample in enumerate(self.cleaned_ecg): # idx starts at 0
            ...
            try:
                ...
                dropped_idxs.append(idx) # note that feature extraction for ecg data at idx was not success
                print(f"Error processing ECG sample {idx} for lead {self.channel}: {e} ")
                continue

        features_df = pd.DataFrame(features_list)
        return features_df, dropped_idxs # return the indxs that did not go through feature extraction
